...
import com.diegotobalina.framework.core.exception.exception.BadRequestException;
...
public class DefaultExceptionHandler {
  ...
  private ErrorResponse methodArgumentNotValidException(MethodArgumentNotValidException ex) {
    ...
    return new ErrorResponse(new Exception(cleanedMessage), HttpStatus.BAD_REQUEST.value())
        .printMessage();
  }

  @ResponseBody
  @ExceptionHandler({RuntimeException.class})
  private ErrorResponse runtimeException(RuntimeException ex) {
    if (ex instanceof ForbiddenTenantException) {
      return new ErrorResponse(ex, HttpStatus.FORBIDDEN.value()).printMessage();
    }
    if (ex instanceof MissingTenantException) {
